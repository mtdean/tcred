...
`NYFED_RECESSION_PROB`, `EBP`, `GZ_SPREAD`, `EBP_REC_PROB`, `NEAR_TERM_FWD_SPREAD` (1980→), `CREDIT_IMPULSE`, `CFSI`, `OFR_FSI` (+5 categories), `BIS_CREDIT_GAP_US`, `NTFS_REC_PROB`, `RECESSION_RISK_ENSEMBLE` (stacked), `RECESSION_RISK_ENSEMBLE_EW` (equal-weight), `USREC`.

**Recession-risk ensemble** (the headline gauge): a **stacked (meta-logit) blend** of three 12-mo recession-probability models — the NY Fed yield-curve probit (`NYFED_RECESSION_PROB`), the EBP probit (`EBP_REC_PROB`), and a Near-Term Forward Spread logit *fit here* against forward-12-month NBER recessions (`USREC`) over 1980–present (≈5 cycles, ~557 obs, stored as `NTFS_REC_PROB`).

- **Weighting (v2).** Instead of an equal-weight mean, the headline is a logistic regression (`_fit_logit`) of forward-12-month NBER recession (`USREC`, fetched/cached as `_fit_ntfs_recession_prob` does) on the **three component probabilities** (as fractions 0–1), over the months where all three exist; the fitted model's prediction is `RECESSION_RISK_ENSEMBLE`. This learns a performance weight per model rather than assuming each is equally informative.
- **Non-negative slopes.** The stack is fit with a projected-Newton **non-negativity constraint** on the three slope coefficients (intercept free): a blended recession probability must be monotone non-decreasing in each component probability, so a negative weight (the symptom of overfitting one false-positive episode) is disallowed. This is the standard forecast-combination guard. Consequence on the current sample: NTFS carries the largest weight, the EBP probit (academically strongest) retains a solidly positive weight (**~+0.94** on the logit scale, *not* down-weighted to zero), and the yield-curve probit clamps to **~0** — economically sensible, because `NYFED_RECESSION_PROB` only exists in the DB from 2015 (its `T10Y3M` input starts then), so over the aligned all-three window it adds nothing beyond NTFS (itself a curve measure) and was the source of the 2022–23 false positive. The stored fitted coefficients are logged on each run.
- **Missing components & fallback.** For months missing a stack feature — notably pre-2015, where `NYFED_RECESSION_PROB` doesn't exist (so the GFC is scored on EBP+NTFS only) — the headline falls back to the **equal-weight mean** of whatever components are present (the v1 behaviour). If the stack as a whole can't be fit — too few aligned obs (`<60`), too few of either target class (`<8`), a singular fit, or all slopes clamping to ~0 — the entire series falls back to equal-weight, exactly as the NTFS probit falls back to its probit guard. The plain equal-weight mean is also stored alongside as **`RECESSION_RISK_ENSEMBLE_EW`** for transparency / comparison.
- **Values.** `RECESSION_RISK_ENSEMBLE` reads ~41% / 36% at 2007-12 / 2009-06 (equal-weight fallback, NYFED absent pre-2015), ~37% at 2019-08 and ~32% at 2023-06 (stacked — note the stack keeps 2023 sane where a naive curve-led model would spike), and ~19% latest.
- **Recession shading (frontend).** The Macro-page "Recession Risk — Ensemble" panel draws the stacked ensemble bold over its components and the equal-weight series, with subtle gray **NBER recession bands** behind the lines. The bands are derived from the `USREC` 0/1 monthly flag (`getFredHistory('USREC')` → `useRecessionIntervals`, which collapses runs of 1s into `[start, end]` spans) and drawn as recharts `<ReferenceArea>` via an optional `shadedIntervals` prop threaded `MultiLineChart` ← `FredSeriesPanel`; the prop is left undefined elsewhere so other panels are unaffected. Intervals are clipped to the visible range so band endpoints stay on the rendered x-domain.
...
3. **Ensemble v2** ✅ — done: meta-logit stacked (performance-weighted, non-negative-slope) blend replaces the equal-weight mean as the headline `RECESSION_RISK_ENSEMBLE`; equal-weight kept as `RECESSION_RISK_ENSEMBLE_EW`; NBER (`USREC`) recession shading added to the chart. *Next:* extend `NYFED_RECESSION_PROB` history pre-2015 (longer `T10Y3M` fetch) so the stack can actually learn a curve-probit weight across the GFC instead of clamping it to ~0 on the post-2015-only window.
